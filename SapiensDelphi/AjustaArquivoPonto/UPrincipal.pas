...
begin
  ...
  if edtCartao.Text = '' then
     begin
       Application.MessageBox('Informe o Cartão','Aviso',MB_ICONWARNING+MB_OK);
  ...
  if Length(edtCartao.Text) <> 5 then
     begin
       Application.MessageBox('O número do cartão está fora do padrão de 5 dígitos','Aviso',MB_ICONWARNING+MB_OK);
       ActiveControl := edtCartao;
       Abort;
     end;
  ...
  if not FileExists(edtArquivo.Text) then
     begin
       Application.MessageBox('Arquivo não encontrado','Aviso',MB_ICONWARNING+MB_OK);
   ...
   if (edtHoraLivre.Text <> '  :  ') and (rbEntrada.Checked = false) and (rbSaida.Checked = false) then
      begin
        Application.MessageBox('Informe se o horário livre é saída ou entrada','Aviso',MB_ICONWARNING+MB_OK);
        Abort;
      end;
  ...
  vaAno := Copy(edtData.Text,9,2);
    ...
    if edtHoraLivre.Text <> '  :  ' then
     ...
     Application.MessageBox('Item Processado','Confirmação',MB_ICONINFORMATION+MB_OK);
