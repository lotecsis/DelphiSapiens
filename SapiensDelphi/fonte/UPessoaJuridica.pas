   ...
   if Application.MessageBox('Deseja Excluir o Registro?', 'Excluir Registro', MB_ICONQUESTION+MB_YESNO) = idyes then
    Begin
     ...
     if not DataModule5.IBQSeExiste.IsEmpty then
      begin
       Application.MessageBox('Cliente está relacionado em algum pedido'#13'e não pode ser excluído', 'Aviso ', MB_ICONWARNING+MB_OK);
       exclui := false;
      end;
     ...
     if not DataModule5.IBQSeExiste.IsEmpty then
      begin
       Application.MessageBox('Representante está relacionado em algum pedido'#13'e não pode ser excluído', 'Aviso ', MB_ICONWARNING+MB_OK);
       exclui := false;
      end;
     ...
     if not DataModule5.IBQSeExiste.IsEmpty then
      begin
       Application.MessageBox('Fornecedor está relacionado em compra de matéria prima'#13'e não pode ser excluído', 'Aviso ', MB_ICONWARNING+MB_OK);
       exclui := false;
      end;
     ...
     if not DataModule5.IBQSeExiste.IsEmpty then
      begin
       Application.MessageBox('Fornecedor está relacionado em Matéria Prima Nf'#13'e não pode ser excluído', 'Aviso ', MB_ICONWARNING+MB_OK);
       exclui := false;
      end;
...
begin
     if DBERzSocial.Text = '' then
        ...
        if DBENmFantasia.Text = '' then
          begin
          MessageDlg('Campo Nome Fantasia é obrigatório.',mtWarning,[mbok],0);
          DBENmFantasia.SetFocus;
          end
        else
        if DBEInscEstadual.Text = '' then
          begin
          MessageDlg('Campo Inscrição Estadual é obrigatório.',mtWarning,[mbok],0);
          DBEInscEstadual.SetFocus;
          end
        else
        if DBECgc.Text = '' then
          begin
          MessageDlg('Campo C.G.C é obrigatório.',mtWarning,[mbok],0);
          DBECgc.SetFocus;
          end
        else
        if DBERamoAtiv.Text = '' then
          begin
          MessageDlg('Campo Ramo de Atividade é obrigatório.',mtWarning,[mbok],0);
          DBERamoAtiv.SetFocus;
          end
        else
        if DBECliFor.Text = '' then
          begin
          MessageDlg('Campo Definição é obrigatório.',mtWarning,[mbok],0);
          DBECliFor.SetFocus;
          end
        else
        if DBECdRegiao.Text = '' then
          begin
          MessageDlg('Campo Região é obrigatório.',mtWarning,[mbok],0);
          DBECdRegiao.SetFocus;
          end
        else
        if DBELiberado.Text = '' then
          begin
          MessageDlg('Campo Liberado? é obrigatório.',mtWarning,[mbok],0);
...
begin
       ...
       begin
        ...
        if DataModule2.IBQConsRegiao.IsEmpty then
          begin
            ShowMessage('Não existe região com este código');
            DBECdRegiao.SetFocus;
            DBECdRegiao.Clear;
          end
        else
...
if Length(DBECgc.Field.AsString) < 14 then
  begin
   MessageDlg('Numero de digitos inferior ao necessário! .',mtWarning,[mbok],0);
   DBECgc.SetFocus;
   Abort;
  end
else
begin
  ...
  //valida o cnpj
   ...
   if not DataModule2.IBQValidaPessoaCnpj.IsEmpty then
    begin
    if Application.MessageBox('Deseja Visualizar a Pessoa?', 'C.N.P.J já existente! ', MB_ICONWARNING+MB_YESNO) = idyes then
    Begin
...
if DataModule2.IBDPessoaJu.IsEmpty then
 begin
  Application.MessageBox('Pessoa não pose ser copiado vazio', 'Aviso ', MB_ICONWARNING+MB_OK);
 end
  else
    begin
