object FTabelaPreco: TFTabelaPreco
  Left = 21
  Top = 20
  BorderIcons = [biSystemMenu]
  Caption = 'Tabela de Pre'#231'o'
  ClientHeight = 555
  ClientWidth = 643
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCloseQuery = FormCloseQuery
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PaginaControle: TPageControl
    Left = 0
    Top = 0
    Width = 643
    Height = 555
    ActivePage = TbProdTab
    Align = alClient
    TabOrder = 0
    TabStop = False
    object TbTabela: TTabSheet
      Caption = 'Tabela'
      OnShow = TbTabelaShow
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object Panel2: TPanel
        Left = 0
        Top = 280
        Width = 521
        Height = 241
        BevelOuter = bvLowered
        TabOrder = 0
        object Label1: TLabel
          Left = 8
          Top = 28
          Width = 33
          Height = 13
          Caption = 'C'#243'digo'
          FocusControl = DBECodigo
        end
        object Label2: TLabel
          Left = 8
          Top = 60
          Width = 48
          Height = 13
          Caption = 'Descri'#231#227'o'
          FocusControl = DBEDescricao
        end
        object Label3: TLabel
          Left = 8
          Top = 156
          Width = 36
          Height = 13
          Caption = 'Usu'#225'rio'
          FocusControl = DBEUsuario
        end
        object Label4: TLabel
          Left = 8
          Top = 92
          Width = 23
          Height = 13
          Caption = 'Sigla'
          FocusControl = DBESigla
        end
        object Label5: TLabel
          Left = 8
          Top = 124
          Width = 48
          Height = 13
          Caption = 'Data Cad.'
        end
        object DBECodigo: TDBEdit
          Left = 64
          Top = 20
          Width = 99
          Height = 21
          TabStop = False
          DataField = 'CD_TABELA'
          DataSource = DSTabPreco
          ReadOnly = True
          TabOrder = 0
        end
        object DBEDescricao: TDBEdit
          Left = 64
          Top = 52
          Width = 349
          Height = 21
          CharCase = ecUpperCase
          DataField = 'DS_TABELA'
          DataSource = DSTabPreco
          TabOrder = 1
        end
        object DBEUsuario: TDBEdit
          Left = 64
          Top = 148
          Width = 99
          Height = 21
          TabStop = False
          DataField = 'USUARIO'
          DataSource = DSTabPreco
          ReadOnly = True
          TabOrder = 2
        end
        object DBESigla: TDBEdit
          Left = 64
          Top = 84
          Width = 199
          Height = 21
          CharCase = ecUpperCase
          DataField = 'SIGLA_TABELA'
          DataSource = DSTabPreco
          TabOrder = 3
        end
        object DBEData: TDBDateEdit
          Left = 64
          Top = 116
          Width = 99
          Height = 21
          TabStop = False
          DataField = 'DT_CADASTRO'
          DataSource = DSTabPreco
          ReadOnly = True
          NumGlyphs = 2
          TabOrder = 4
        end
        object Panel3: TPanel
          Left = 8
          Top = 184
          Width = 505
          Height = 49
          BevelOuter = bvLowered
          TabOrder = 5
        end
      end
      object BSair: TRbButton
        Left = 446
        Top = 472
        Width = 49
        Height = 30
        Color = clMenuBar
        ParentColor = False
        OnClick = BSairClick
        TabOrder = 1
        TabStop = False
        TextShadow = True
        ModalResult = 0
        Glyph.Data = {
          
          76820000000000000000}
        Spacing = 2
        Layout = blGlyphLeft
        Colors.DefaultFrom = clWhite
        Colors.DefaultTo = 13745839
        Colors.OverFrom = clWhite
        Colors.OverTo = 12489846
        Colors.ClickedFrom = 13029334
        Colors.ClickedTo = 15463415
        Colors.HotTrack = clBlue
        Colors.BorderColor = clGray
        Colors.TextShadow = clWhite
        FadeSpeed = fsMedium
        ShowFocusRect = True
        HotTrack = False
        GradientBorder = True
      end
      object BInserir: TRbButton
        Left = 16
        Top = 472
        Width = 75
        Height = 30
        OnClick = BInserirClick
        TabOrder = 2
        TextShadow = True
        Caption = 'Inserir'
        ModalResult = 0
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFF0000}
        Spacing = 2
        Layout = blGlyphLeft
        Colors.DefaultFrom = clWhite
        Colors.DefaultTo = 13745839
        Colors.OverFrom = clWhite
        Colors.OverTo = 12489846
        Colors.ClickedFrom = 13029334
        Colors.ClickedTo = 15463415
        Colors.HotTrack = clBlue
        Colors.BorderColor = clGray
        Colors.TextShadow = clWhite
        FadeSpeed = fsMedium
        ShowFocusRect = True
        HotTrack = False
        GradientBorder = True
      end
      object BCancelar: TRbButton
        Left = 102
        Top = 472
        Width = 75
        Height = 30
        OnClick = BCancelarClick
        TabOrder = 3
        TabStop = False
        TextShadow = True
        Caption = 'Cancelar'
        ModalResult = 0
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFF0000}
        Spacing = 2
        Layout = blGlyphLeft
        Colors.DefaultFrom = clWhite
        Colors.DefaultTo = 13745839
        Colors.OverFrom = clWhite
        Colors.OverTo = 12489846
        Colors.ClickedFrom = 13029334
        Colors.ClickedTo = 15463415
        Colors.HotTrack = clBlue
        Colors.BorderColor = clGray
        Colors.TextShadow = clWhite
        FadeSpeed = fsMedium
        ShowFocusRect = True
        HotTrack = False
        GradientBorder = True
      end
      object BConfirmar: TRbButton
        Left = 188
        Top = 472
        Width = 75
        Height = 30
        OnClick = BConfirmarClick
        TabOrder = 4
        TextShadow = True
        Caption = 'Confirmar'
        ModalResult = 0
        Glyph.Data = {
          
          FFFFFDFEFDFEFEFE0000}
        Spacing = 2
        Layout = blGlyphLeft
        Colors.DefaultFrom = clWhite
        Colors.DefaultTo = 13745839
        Colors.OverFrom = clWhite
        Colors.OverTo = 12489846
        Colors.ClickedFrom = 13029334
        Colors.ClickedTo = 15463415
        Colors.HotTrack = clBlue
        Colors.BorderColor = clGray
        Colors.TextShadow = clWhite
        FadeSpeed = fsMedium
        ShowFocusRect = True
        HotTrack = False
        GradientBorder = True
      end
      object BAlterar: TRbButton
        Left = 274
        Top = 472
        Width = 75
        Height = 30
        OnClick = BAlterarClick
        TabOrder = 5
        TabStop = False
        TextShadow = True
        Caption = 'Alterar'
        ModalResult = 0
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFF0000}
        Spacing = 2
        Layout = blGlyphLeft
        Colors.DefaultFrom = clWhite
        Colors.DefaultTo = 13745839
        Colors.OverFrom = clWhite
        Colors.OverTo = 12489846
        Colors.ClickedFrom = 13029334
        Colors.ClickedTo = 15463415
        Colors.HotTrack = clBlue
        Colors.BorderColor = clGray
        Colors.TextShadow = clWhite
        FadeSpeed = fsMedium
        ShowFocusRect = True
        HotTrack = False
        GradientBorder = True
      end
      object BExcluir: TRbButton
        Left = 360
        Top = 472
        Width = 75
        Height = 30
        OnClick = BExcluirClick
        TabOrder = 6
        TabStop = False
        TextShadow = True
        Caption = 'Excluir'
        ModalResult = 0
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFF0000}
        Spacing = 2
        Layout = blGlyphLeft
        Colors.DefaultFrom = clWhite
        Colors.DefaultTo = 13745839
        Colors.OverFrom = clWhite
        Colors.OverTo = 12489846
        Colors.ClickedFrom = 13029334
        Colors.ClickedTo = 15463415
        Colors.HotTrack = clBlue
        Colors.BorderColor = clGray
        Colors.TextShadow = clWhite
        FadeSpeed = fsMedium
        ShowFocusRect = True
        HotTrack = False
        GradientBorder = True
      end
      object Panel1: TPanel
        Left = 0
        Top = 8
        Width = 633
        Height = 265
        BevelOuter = bvLowered
        TabOrder = 7
        object DBGTabPreco: TDBGrid
          Left = 5
          Top = 6
          Width = 628
          Height = 252
          DataSource = DSGridTabPreco
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
          ReadOnly = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnDblClick = DBGTabPrecoDblClick
          Columns = <
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'CD_TABELA'
              Title.Alignment = taCenter
              Title.Caption = 'C'#243'digo'
              Width = 70
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'DS_TABELA'
              Title.Alignment = taCenter
              Title.Caption = 'Descri'#231#227'o'
              Width = 300
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'SIGLA_TABELA'
              Title.Alignment = taCenter
              Title.Caption = 'Sigla'
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'DT_CADASTRO'
              Title.Alignment = taCenter
              Title.Caption = 'Data Cad.'
              Width = 80
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'USUARIO'
              Title.Alignment = taCenter
              Title.Caption = 'Usu'#225'rio'
              Width = 130
              Visible = True
            end>
        end
      end
      object BImprimir: TRbButton
        Left = 536
        Top = 296
        Width = 85
        Height = 30
        OnClick = BImprimirClick
        TabOrder = 8
        TextShadow = True
        Caption = 'Imprimir'
        ModalResult = 0
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFF0000}
        Spacing = 2
        Layout = blGlyphLeft
        Colors.DefaultFrom = clWhite
        Colors.DefaultTo = 13745839
        Colors.OverFrom = clWhite
        Colors.OverTo = 12489846
        Colors.ClickedFrom = 13029334
        Colors.ClickedTo = 15463415
        Colors.HotTrack = clBlue
        Colors.BorderColor = clGray
        Colors.TextShadow = clWhite
        FadeSpeed = fsMedium
        ShowFocusRect = True
        HotTrack = False
        GradientBorder = True
      end
    end
    object TbProdTab: TTabSheet
      Caption = 'Produtos'
      ImageIndex = 1
      OnShow = TbProdTabShow
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object DBGProdTabela: TDBGrid
        Left = 0
        Top = 64
        Width = 633
        Height = 393
        DataSource = DSGridProdTab
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'CD_PRODUTO'
            Title.Alignment = taCenter
            Title.Caption = 'C'#243'digo'
            Width = 80
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DS_PRODUTO'
            Title.Alignment = taCenter
            Title.Caption = 'Descri'#231#227'o'
            Width = 350
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'PRECO_VENDA'
            Title.Alignment = taCenter
            Title.Caption = 'Pre'#231'o'
            Width = 80
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'CUBICO'
            Title.Alignment = taCenter
            Title.Caption = 'C'#250'bico'
            Width = 70
            Visible = True
          end>
      end
      object Panel4: TPanel
        Left = 64
        Top = 464
        Width = 505
        Height = 49
        BevelOuter = bvLowered
        TabOrder = 1
        object BAdicionaProd: TRbButton
          Left = 128
          Top = 11
          Width = 75
          Height = 30
          OnClick = BAdicionaProdClick
          TabOrder = 0
          TextShadow = True
          Caption = 'Adiciona'
          ModalResult = 0
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFF0000}
          Spacing = 2
          Layout = blGlyphLeft
          Colors.DefaultFrom = clWhite
          Colors.DefaultTo = 13745839
          Colors.OverFrom = clWhite
          Colors.OverTo = 12489846
          Colors.ClickedFrom = 13029334
          Colors.ClickedTo = 15463415
          Colors.HotTrack = clBlue
          Colors.BorderColor = clGray
          Colors.TextShadow = clWhite
          FadeSpeed = fsMedium
          ShowFocusRect = True
          HotTrack = False
          GradientBorder = True
        end
        object BAlteraProd: TRbButton
          Left = 216
          Top = 11
          Width = 75
          Height = 30
          OnClick = BAlteraProdClick
          TabOrder = 1
          TextShadow = True
          Caption = 'Altera'
          ModalResult = 0
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFF0000}
          Spacing = 2
          Layout = blGlyphLeft
          Colors.DefaultFrom = clWhite
          Colors.DefaultTo = 13745839
          Colors.OverFrom = clWhite
          Colors.OverTo = 12489846
          Colors.ClickedFrom = 13029334
          Colors.ClickedTo = 15463415
          Colors.HotTrack = clBlue
          Colors.BorderColor = clGray
          Colors.TextShadow = clWhite
          FadeSpeed = fsMedium
          ShowFocusRect = True
          HotTrack = False
          GradientBorder = True
        end
        object BExcluiProd: TRbButton
          Left = 304
          Top = 11
          Width = 75
          Height = 30
          OnClick = BExcluiProdClick
          TabOrder = 2
          TextShadow = True
          Caption = 'Exclui'
          ModalResult = 0
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFF0000}
          Spacing = 2
          Layout = blGlyphLeft
          Colors.DefaultFrom = clWhite
          Colors.DefaultTo = 13745839
          Colors.OverFrom = clWhite
          Colors.OverTo = 12489846
          Colors.ClickedFrom = 13029334
          Colors.ClickedTo = 15463415
          Colors.HotTrack = clBlue
          Colors.BorderColor = clGray
          Colors.TextShadow = clWhite
          FadeSpeed = fsMedium
          ShowFocusRect = True
          HotTrack = False
          GradientBorder = True
        end
      end
      object EdDsTabela: TEdit
        Left = 104
        Top = 8
        Width = 497
        Height = 27
        TabStop = False
        Color = clMoneyGreen
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        ReadOnly = True
        TabOrder = 2
      end
      object EdDsCdTabela: TEdit
        Left = 8
        Top = 8
        Width = 89
        Height = 27
        TabStop = False
        Color = clMoneyGreen
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        ReadOnly = True
        TabOrder = 3
      end
    end
  end
  object DSTabPreco: TDataSource
    AutoEdit = False
    OnStateChange = DSTabPrecoStateChange
    Left = 456
    Top = 8
  end
  object DSGridTabPreco: TDataSource
    Left = 420
    Top = 8
  end
  object DSGridProdTab: TDataSource
    Left = 364
    Top = 16
  end
end
